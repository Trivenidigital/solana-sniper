...
async def _extract_bought_token(
    signature: str, wallet: str, settings: Settings, session: aiohttp.ClientSession,
) -> str | None:
                                ...
                                if stale > 1800:
                                    ...
                                    if send_telegram_fn:
                                        await send_telegram_fn(
                                            f"Smart Money WebSocket may be down\nNo signals in {int(stale/60)} min",
                                            settings,
                                        )
                                        last_injection_time = datetime.now(timezone.utc)

                        heartbeat_task = asyncio.create_task(_heartbeat_check())
